...
@section('title_dashboard',"Edit $route Menu")
@section('dashcontent')
...
<section class="row">
    ...
    <div class="col-12 bg-white p-5 rounded-3">
        <form action="{{ route('aldev'.$route.'.update',$data['id']) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            @for ($i=0;$i < count($dn);$i++)
                @if (in_array($input_data[$i]->input_type,['text','number','password','email']))
                    @include('forms.input',['label'=>$input_data[$i]->input_label,'name'=>$dn[$i]->data_name,'placeholder'=>$input_data[$i]->input_label,'value'=>old($input_data[$i]->input_type,$data[$dn[$i]->data_name]),'type'=>$input_data[$i]->input_type])
                @elseif ($input_data[$i]->input_type == 'tiny')
                    @include('forms.tinycode',['code_name'=>$dn[$i]->data_name,'code_title'=>$input_data[$i]->input_label])
                @endif
            @endfor
            
            <button type="submit" class="btn btn-primary mt-4">Perbarui {{ ucfirst($route) }}</button>
        </form>
    </div>
</section>
@endsection